...
There is no universal coverage number. The right threshold depends on the code's risk and nature — critical business logic warrants more, trivial wrappers less. The orchestrator sets the target per module. **80% line coverage** is a reasonable default for business logic, but treat it as a guide, not a finish line: coverage proves code was executed, not that the assertions catch regressions. That second question is what `test-input-auditor` answers via mutation-thinking — a high-coverage suite of Liar tests is worse than a smaller suite that actually fails when behavior breaks.
...
Cover high-risk and frequently-changed code first. Use version control history to find churn hot spots (most recently and most frequently changed files) and put those under test before low-traffic code. A small set of tests on critical paths beats broad shallow coverage.
