...
**Verification clause for transferable practices.** When the information to persist is a practice about an external tool, framework, or language (as opposed to a project-internal fact or convention), launch the `practice-verifier` agent with the claim before writing — what just worked in this session may be version-specific, incomplete, or accidentally right, and a persisted mistake propagates to every future session. Persist only what the agent deems actionable per its own contract (`agents/practice-verifier.md` owns the confidence bar), applying its corrections when the verdict is `adjusted`; report refuted claims to the user with the verifier's evidence. Skip verification for project-internal facts — the codebase, not the internet, is their authority, and the session just observed them directly.
