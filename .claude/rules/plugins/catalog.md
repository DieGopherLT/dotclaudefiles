...
The full harness for a substantial task: one lifecycle, chainable skills that each stand alone, and two execution modes over the same process — in-session (`task-execution`) or by delegation (`mastermind-role`). The split exists because the phases have different natures — in particular, the quality gate is runnable against any branch without going through planning:

- **Skills**: `task-planning` (git state with the branch trigger keyed to protected branches, worktree decided by countable checks — a bundled `detect-concurrent-claudes.sh` yes/no script, >=3 letter groups, >7 files — letter-group breakdown with group codes as mandatory title tokens, an `A0` task recording the base ref, `TaskCreate` registration; hands off to `task-execution`), `task-execution` (LSP-first navigation, a per-subtask dispatch rubric — the discard test plus the prompt test decide mechanically between main context / full delegation / partial delegation / fan-out `Workflow`, with delegated tasks claimed via `TaskUpdate` `owner` — commits at group boundaries, immediate `TaskUpdate`, countable friction stops — same command failing 3 times or unplanned subtasks outnumbering a group's registered ones ends the turn; hands off to `task-quality-gate`), `task-quality-gate` (generates the patch once via `prepare-patch.sh` — merge-base-resolved so an advanced base cannot pollute it, base ref read from planning's `A0` task with a standalone fallback — derives an effort band from a base-band table plus ±1 modifiers, passes external domain auditors as `extraAuditors` into the review `Workflow`, arbitrates in-scope vs pre-existing findings, reports via `ReportFindings` with a deterministic class/verdict severity order, and dispatches fixes), `mastermind-role` (`disable-model-invocation`: user-only via `/mastermind-role`; installs the orchestrator role — main context reserved for reasoning and integration with an inline threshold of 1-2 files / ~40 LoC, mandatory dedicated worktree with a `.worktreeinclude` preflight, ledger of task FSMs `pending -> dispatched -> reported -> in-review -> integrated | blocked`, tier dispatch with specialists preferred over generic workers, write-concurrency isolation rule — parallel write waves get per-worker worktrees because a shared git index is unsafe even with disjoint files — Dev Lead report contract injected into every dispatch, `Workflow` at the same >=12-item cutoff as task-execution, closes through task-quality-gate; persistence via the shared `binnacle` skill, resumed by a later `/mastermind-role` invocation in the same worktree), `binnacle` (shared durable run log for both execution modes: `.claude/binnacle.md` per worktree from a bundled template, ignored via `.git/info/exclude` — never the tracked `.gitignore` — rewritten at every task transition with a Resume-with entry point; opened by worktree runs of task-planning and mastermind-role, mirrored by task-execution at group boundaries, and restored by a cold session before resuming; plain-branch runs deliberately get none)
- **Workers** (4, write-capable with commit authority, all reporting macro-contextually — outcome / commit SHAs / contract deltas / deviations / needs — with detail addressable via `git show`): `light-worker` (haiku: deterministic fully-specified tasks, halts on any unresolved decision), `scout-worker` (sonnet low: scale and exploration-with-synthesis), `standard-worker` (sonnet medium: baseline module/feature against fixed contracts), `smart-worker` (opus high: high cognitive load, documents its design decisions)
