...
Turns passive Clean Code reference into a reactive analysis of real code: given a file, directory, or symbol, it detects concrete smells from the refactoring.guru taxonomy, maps them to named techniques, and applies the chosen transformation step by step:

- **Agents**: `smell-detector` (read-only Auditor, one per category, runs in parallel inside the smell-scan Workflow; confidence-scored located findings at >=80, never modifies files), `refactoring-applier` (Implementer: applies ONE named technique to ONE location following the playbook mechanics, preserves observable behavior, reports the verification)
- **Skills**: `smell-scan` (orchestrator: fans 5 `smell-detector` agents across the 5 categories in parallel via a Workflow, synthesizes a priority-ordered report in plain JS with per-scan reference codes B1/OO1/CP1/D1/C1), `refactor` (coordinator: resolves a finding code or explicit technique+location, loads the technique playbook, guards with tests, dispatches the applier, verifies behavior preserved)
