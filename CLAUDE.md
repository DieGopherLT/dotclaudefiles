...
Turns passive Clean Code reference into a reactive analysis of real code: given a file, directory, or symbol, it detects concrete smells from the refactoring.guru taxonomy, maps them to named techniques, and applies the chosen transformation step by step. Both skills are scale dispatchers: a target spanning one physical domain (a top-level module folder) runs inline; a target spanning two or more domains fans out through a `Workflow`:

- **Agents**: `smell-detector` (read-only Auditor, one per category, runs per domain — inline via the `Agent` tool for a single domain or inside the smell-scan Workflow's per-domain pipeline for multi-domain scans; confidence-scored located findings at >=80 with a `resolution_plan` per finding, never modifies files), `refactoring-applier` (Implementer: applies ONE named technique to ONE location following the playbook mechanics, preserves observable behavior, reports the verification; frozen, never touches git), `refactoring-reconciler` (Bash-capable: owns all git/build mechanics inside the refactor Workflow's Mark and Reconcile phases — ephemeral rollback commit, whole-project build gate, reset on failure — never edits code)
- **Skills**: `smell-scan` (dispatcher: partitions the target into domains, runs a 5-`smell-detector` batch per domain — inline for one domain, pipelined via a Workflow for N>=2 — then assigns globally-unique reference codes B1/OO1/CP1/D1/C1 over the aggregated findings and persists one source-of-truth JSON file per domain under `.claude/refactoring-guru/findings/`), `refactor` (dispatcher: resolves a finding code, explicit technique+location, or a broader multi-domain request; a single finding runs inline through the existing test → refactor → test → commit cycle, while N>=2 domains fan out inside a dedicated worktree — parallel intra-domain appliers, cross-cutting smells reconciled serially with a build gate, collapsed to one `refactor:` commit)
