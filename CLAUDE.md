...
Turns passive Clean Code reference into a reactive analysis of real code: given a file, directory, or symbol, it detects concrete smells from the refactoring.guru taxonomy, maps them to named techniques, and applies the chosen transformation step by step. `smell-scan` always fans out through a `Workflow` regardless of domain count (detection is read-only, so there is no worktree-isolation reason to special-case a single domain); `refactor` remains a scale dispatcher — a single finding runs inline, two or more domains fan out inside a dedicated worktree, since only mutation needs multi-writer isolation:
