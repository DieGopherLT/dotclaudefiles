...
- **Config**: optional per-project `.claude/claude-management.local.json` — keys: `claudemd_ceiling`, `failure_min_repeats`, `contextualizable.roots`/`.min_source_files`/`.min_touched_files`/`.source_extensions`/`.iac_extensions`, `harvest.queue_threshold`/`.min_transcript_bytes`/`.max_user_messages`; harvest queue state in `~/.claude/claude-management/harvest/`
